...
class Parser:
	...
	def power(self):
		return self.bin_op(self.atom, (TT_POW, TT_ROOT), self.factor)
...
class Number:
	...
	def dived_by(self, other):
		if isinstance(other, Number):
			if other.value == 0:
				return None, RTError(
					other.pos_start, other.pos_end, DivisionByZeroMessage[CURRENT_LANG], self.context
				)
...
class Interperter:
	def visit(self, node, context):
		...
		method = getattr(self, method_name, self.no_visited_method)
		return method(node, context)

	def no_visited_method(self, node, context):
